    ...
    return (
        <>
            ...
            <div className="flex flex-col h-full font-win">
                <div ref={desktopRef} className="flex relative bg-win-blue overflow-hidden block h-full">
                    <div className="absolute p-12 inset-0 flex items-center justify-center">
                        <img className="w-64 opacity-75" src="/img/roses-not-rifles-v4.png" alt="" />
                    </div>
                    ...
                    <div className="absolute bottom-0 right-0 p-4">
                        <a target="_blank" href="https://sebastianalsina.com" className="block border-emboss text-xs transform scale-75 origin-bottom-right opacity-50 hover:opacity-100 text-white p-1">
                            {'</>'} by Sebastian Alsina
                        </a>
                    </div>
                    <div className="absolute z-40 pointer-events-none inset-0">{showStart && <StartMenu shutdown={doShutdown} toggle={() => setShowStart((s) => !s)} />}</div>
                </div>
                <Taskbar>
                        ...
                        return (
                            <Button
                                ...
                                onClick={() => {
                                    ...
                                    const setTop = (value) => {
                                        popRefs.current[window.id].setTop(value)
                                    }
                            ...
                            >
                                {window.title.length >= 22 ? window.title.slice(0, 22) + '...' : window.title}
                            </Button>
                        )
                    })}
