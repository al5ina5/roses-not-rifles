...
export default function StartMenu({ shutdown, toggle }) {
    ...
    return (
        <>
            <div ref={startRef} className='z-50 pointer-events-auto max-w-sm w-full absolute p-1 bottom-0'>
                <div className='w-full bg-win-gray border-emboss'>
                    ...
                    <div className='h-full overflow-auto'>
                        {isListing && <Listing shutdown={shutdown} setActiveTab={setActiveTab} />}
                        {isContact && <Contact />}
                        {isAbout && <About />}
                    </div>
                </div>
