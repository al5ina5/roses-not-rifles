...
function reducer(initalState, action) {
    ...
    switch (action.type) {
        ...
        case "cart.remove":
            ...
            const removedItem = state.line_items.filter(
...
export default function ShopWindow({ close, desktopRef }) {
    ...
    return (
        <ShopProvider value={{ state, dispatch }}>
            ...
            > */}
            <>
                <div className="bg-win-gray border-emboss mb-2 text-xs p-2 space-x-2">
                    <a
                        className={isProducts ? "underline" : ""}
                    ...
                    <a
                        className={isCheckout ? "underline" : ""}
                        onClick={() => setActiveTab(CHECKOUT_TAB)}
                    >
                        Checkout
                    </a>
                </div>
